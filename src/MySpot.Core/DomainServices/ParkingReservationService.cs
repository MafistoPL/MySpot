...
internal sealed class ParkingReservationService : IParkingReservationService
{
    ...
    public void ReserveParkingForCleaning(IEnumerable<WeeklyParkingSpot> allParkingSpots, Date date)
    {
        ...
        {
            var reservationsForSameDate = 
                parkingSpot.Reservations.Where(x => x.Date == date);
            
            parkingSpot.RemoveReservations(reservationsForSameDate);

            const int fullCapacity = WeeklyParkingSpot.MaxParkingSpotCapacity;
            var cleaningReservation = new CleaningReservations(
                ReservationId.Create(), parkingSpot.Id, fullCapacity, date);
